...
export default function WaiterDashboard() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 min-h-0 flex overflow-x-auto pb-6 pt-2 gap-6 snap-x snap-mandatory lg:flex lg:flex-nowrap lg:space-x-6 hide-scrollbar">
        {(() => {
          const mainCols = COLUMNS.map(col => ({ 
            ...col, 
            orders: orders.filter(o => o.status === col.id) 
          }));
          const unsortedOrders = orders.filter(o => !['Pending', 'InKitchen', 'Ready', 'Served', 'Completed'].includes(o.status));
          ...
          return finalCols.map(col => {
            ...
            return (
              <div key={col.id} className="flex flex-col h-full bg-gray-100/50 rounded-3xl p-4 border border-gray-200/60 w-[85vw] shrink-0 snap-center lg:w-80">
                  ...
                  {colOrders.map(order => (
                    <div key={order.id} className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-all flex flex-col justify-between group">
                      ...
                      <div className="pt-3 border-t border-gray-100">
                        ...
                        {col.id === 'Unsorted' && (
                          <button onClick={() => updateStatus(order.id, 'Pending')} className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-2.5 rounded-xl transition-colors text-sm">
                            Initialize Order
                          </button>
                        )}
                        {col.id === 'InKitchen' && (
