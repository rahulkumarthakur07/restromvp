...
export default function OrderHistory() {
  ...
  const filteredOrders = orders.filter((o) => {
    ...
    if (searchQuery) {
      ...
      const matchToken = String(o.tokenNumber).includes(lowerQuery);
      const matchCustomer = o.customerName && o.customerName.toLowerCase().includes(lowerQuery);
      const matchTable = String(o.tableId).toLowerCase().includes(lowerQuery);
      matchQuery = matchToken || matchCustomer || matchTable;
    }
    ...
    if (startDate || endDate) {
      if (!o.timestamp) {
        ...
        const orderDateStr = new Date(o.timestamp.toDate()).toISOString().split("T")[0];
    ...
    totalPages > 1 ? (
      <div className="flex flex-col md:flex-row items-center justify-between p-4 border-t border-gray-50 gap-3">
        <p className="text-xs font-bold text-gray-400">
          ...
          of <span className="font-black text-gray-700">{filteredOrders.length}</span>{" "}
          orders
        </p>
        <div className="flex items-center gap-1.5 flex-wrap justify-center">
          <button
            ...
            className="px-4 py-2 rounded-2xl text-xs font-black bg-gray-100 hover:bg-gray-200 text-gray-600 disabled:opacity-30 transition-all active:scale-95"
          >
            ← Prev
          </button>
          {Array.from({ length: totalPages }, (_, i) => i + 1)
            ...
            .map((p, idx) =>
              p === "..." ? (
                <span key={`d${idx}`} className="px-2 text-gray-300 font-black">
                  …
                </span>
              ) : (
                <button
                  ...
                  className={`w-9 h-9 rounded-2xl text-xs font-black transition-all active:scale-95 ${currentPage === p ? "bg-gray-950 text-white shadow-md" : "bg-gray-100 hover:bg-gray-200 text-gray-600"}`}
                >
                  {p}
                </button>
              ),
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-gray-400 font-medium">
              Complete record of every order placed in the system.
            </p>
          </div>
          <div className="flex flex-col xl:flex-row items-center gap-3 shrink-0 w-full xl:w-auto mt-4 xl:mt-0">
            <div className="flex w-full xl:w-auto gap-2">
              <input
                type="date"
                value={startDate}
                onChange={(e) => { setStartDate(e.target.value); setCurrentPage(1); }}
                className="w-1/2 xl:w-36 px-3 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-[13px] font-bold outline-none focus:border-blue-400 transition-colors text-gray-600"
              />
              <input
                type="date"
                value={endDate}
            ...
            <input
              ...
              className="w-full xl:w-64 px-4 py-3 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold outline-none focus:border-blue-400 transition-colors"
            />
            <div className="flex flex-col items-center justify-center bg-white border border-gray-100 rounded-3xl px-6 py-3 shadow-sm min-w-[100px] w-full xl:w-auto">
              ...
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest mt-0.5">
                {(searchQuery || startDate || endDate) ? "Found" : "Total Logged"}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="hidden md:block bg-white rounded-4xl border border-gray-100 shadow-sm overflow-hidden">
        {orders.length === 0 ? (
          <div className="py-24 flex flex-col justify-center items-center">
            ...
            <p className="font-black text-gray-300 text-xl mb-2">
              No orders yet
            </p>
                    ...
                    return (
                      <React.Fragment key={order.id}>
                        ...
                        >
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 rounded-2xl bg-gray-50 border border-gray-100 flex flex-col items-center justify-center shrink-0">
                                <span className="text-[8px] font-black text-gray-300 uppercase leading-none">
                                  SN
                                </span>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap font-black text-gray-950 text-sm">
                            {order.customerName || "Guest"}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-xs font-black px-3 py-1.5 bg-blue-50 text-blue-600 rounded-2xl border border-blue-100">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div>
                              ...
                              <span
                                className={`block text-[10px] font-black uppercase mt-0.5 ${order.paid ? "text-emerald-600" : "text-red-400"}`}
                              >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-xs font-black text-gray-900 capitalize px-3 py-1.5 bg-gray-50 rounded-2xl border border-gray-100">
                              {order.paymentMethod === 'udhar' ? 'Credit' : (order.paymentMethod || '—')}
                            </span>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span
                              className={`text-[10px] font-black px-3 py-1.5 rounded-2xl border uppercase tracking-tight ${st.bg} ${st.text} ${st.border}`}
                            >
                              {st.label}
                            </span>
                          </td>
                          ...
                          <td
                            className="px-6 py-4 whitespace-nowrap text-right space-x-1"
                            onClick={(e) => e.stopPropagation()}
                          >
                            <button
                              onClick={() => handlePrint(order)}
                              className="p-2.5 rounded-2xl hover:bg-blue-50 text-gray-300 hover:text-blue-600 transition-all active:scale-95 border border-gray-100"
                            >
                            ...
                            <button
                              ...
                              className="p-2.5 rounded-2xl hover:bg-gray-100 text-gray-300 hover:text-gray-700 transition-all active:scale-95 border border-gray-100"
                            >
                              <Download className="w-4 h-4" />
                            </button>
                            <button
                              onClick={() => handleDeleteOrder(order.id)}
                              className="p-2.5 rounded-2xl hover:bg-red-50 text-gray-300 hover:text-red-500 transition-all active:scale-95 border border-gray-100"
                            >
                            ...
                            <td colSpan={7} className="px-6 py-4">
                              <div className="space-y-2">
                                {order.items?.map((item, idx) => (
                                  <div
                                    key={idx}
                                    className="flex justify-between text-xs font-bold text-gray-700 bg-white p-3 rounded-xl border border-gray-100"
                                  >
                                    <span>
                                  ...
                                  <div className="mt-2 pt-2 border-t border-blue-100 space-y-1 px-1">
                                    {order.serviceChargeAmount > 0 && (
                                      <div className="flex justify-between text-[10px] font-black text-amber-600 uppercase tracking-widest">
                                        ...
                                        <span>
                                          {settings.currency || "Rs."}{" "}
                                          {order.serviceChargeAmount.toFixed(0)}
                                        </span>
                                      </div>
                                    )}
                                    {order.taxAmount > 0 && (
                ...
                >
                  <div className="p-5 flex justify-between items-start">
                    <div className="flex gap-3.5">
                      ...
                      <div>
                        <h3 className="font-black text-gray-950 leading-tight mb-1.5">
                          {order.customerName || "Guest"}
                        </h3>
                        <div className="flex items-center gap-2 flex-wrap">
                          <span
                            className={`text-[10px] font-black px-2.5 py-1 rounded-xl border uppercase tracking-tight ${st.bg} ${st.text} ${st.border}`}
                          >
                            {st.label}
                          </span>
                    ...
                    <div className="text-right shrink-0 ml-2">
                      <div className="font-black text-gray-950 text-base leading-none mb-1">
                        {settings.currency || "Rs."}
                        {order.totalAmount?.toFixed(0)}
                      </div>
                      <div
                        className={`text-[9px] font-black uppercase tracking-widest ${order.paid ? "text-emerald-600" : "text-red-400"}`}
                      >
                        {order.paid ? "Paid" : "Unpaid"}
                      </div>
                    </div>
                    ...
                    <div className="px-5 pb-5 border-t border-gray-50 animate-in slide-in-from-top-1 duration-300 pt-4 space-y-3">
                      <div className="space-y-2">
                        {order.items?.map((item, idx) => (
                          <div
                            key={idx}
                            className="flex justify-between text-xs font-bold text-gray-700 bg-gray-50 p-3 rounded-2xl border border-gray-100"
                          >
                            <span>
                      ...
                      >
                        <button
                          onClick={() => handlePrint(order)}
                          className="flex-1 bg-white border border-gray-200 py-3 rounded-2xl font-black text-xs flex items-center justify-center gap-1.5 shadow-sm active:scale-95 transition-all"
                        >
                          <Printer className="w-3.5 h-3.5 text-blue-500" />{" "}
                          Print
                        </button>
                        <button
                          onClick={() => generatePDFReceipt(order, settings)}
                          className="flex-1 bg-gray-950 text-white py-3 rounded-2xl font-black text-xs flex items-center justify-center gap-1.5 shadow-lg active:scale-95 transition-all"
                        >
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((p, idx) =>
                      p === "..." ? (
                        <span
                          key={`d${idx}`}
                          className="px-2 text-gray-300 font-black"
                        >
                          …
                        </span>
                      ) : (
