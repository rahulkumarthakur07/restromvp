...
export default function Subscription() {
    ...
    const unsubSub = onSnapshot(
      ...
      (docSnap) => {
        if (docSnap.exists()) {
          ...
          setDoc(doc(db, "subscriptions", "status"), defaultSub).catch((e) =>
            console.error("Status init failed:", e),
          );
        }
      },
    );
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (config?.flashSaleExpiry) {
        const expiry = config.flashSaleExpiry.toDate
          ? config.flashSaleExpiry.toDate()
          : new Date(config.flashSaleExpiry);
    ...
    const interval = setInterval(() => {
      if (config?.recentLocations?.length > 0) {
        const city =
          config.recentLocations[
            Math.floor(Math.random() * config.recentLocations.length)
          ];
        setSocialProof(
          `📍 A restaurant in ${city} just upgraded to Pro Yearly!`,
        );
  ...
  const handleWhatsApp = () => {
    const message = encodeURIComponent(
      `Hi RestroMVP Support! I want to upgrade to the ${selectedPlan.name} (${selectedPlan.period}) plan. Please guide me on payment.`,
    );
  ...
  const handleEmail = () => {
    const subject = encodeURIComponent(
      `Subscription Upgrade: ${selectedPlan.name}`,
    );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 md:space-y-10 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700 relative">
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-300 border border-gray-100">
            <div className="relative p-8 md:p-10 space-y-8">
              ...
              <div className="space-y-4 text-center">
                <div
                  className={`w-20 h-20 mx-auto rounded-3xl flex items-center justify-center bg-linear-to-br ${selectedPlan.color} text-white shadow-xl`}
                >
                  {selectedPlan.recommended ? (
                    <Crown className="w-10 h-10" />
                  ) : selectedPlan.name.includes("Yearly") ? (
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-gray-500 font-medium">
                    Contact us to finalize your upgrade
                  </p>
                </div>
              </div>
              ...
              <div className="text-center pt-4 border-t border-gray-50">
                ...
                <p className="text-gray-950 font-bold text-sm select-all">
                  {CONTACT_EMAIL}
                </p>
      ...
      <div className="bg-linear-to-r from-red-600 via-orange-600 to-red-600 py-3 rounded-2xl text-white text-center shadow-lg shadow-red-100 flex flex-col md:flex-row items-center justify-center gap-2 md:gap-6 group">
        ...
        <span className="text-xs font-bold opacity-90 hidden md:block tracking-widest uppercase italic">
          Price Increase Imminent!
        </span>
      </div>

      {/* Premium Hero Status Section */}
      <div className="relative group overflow-hidden bg-white rounded-4xl border border-gray-100 shadow-xl shadow-gray-200/50 p-6 md:p-12">
        <div className="absolute top-0 right-0 -mr-20 -mt-20 w-80 h-80 bg-blue-100/30 rounded-full transition-transform group-hover:scale-125 duration-1000" />
        <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-80 h-80 bg-purple-100/30 rounded-full transition-transform group-hover:scale-125 duration-1000" />
        ...
        <div className="relative flex flex-col md:flex-row items-center justify-between gap-10">
          <div className="space-y-6 text-center md:text-left flex-1">
            ...
            <div className="space-y-2">
              <h1 className="text-3xl md:text-5xl font-black text-gray-950 leading-[1.1]">
                {subData?.plan
                  ? `Active Plan: ${subData.plan}`
                  : "Experience Premium Tools"}
              </h1>
              <p className="text-gray-500 max-w-lg text-base md:text-lg font-medium">
                Unlock full features, premium support, and scaling tools
          ...
          <div className="w-full md:w-auto">
            {subData?.expiryDate || subData?.plan?.includes("Starter") ? (
              <div
                className={`p-8 md:p-10 rounded-4xl border-2 transition-all relative overflow-hidden flex flex-col items-center justify-center min-w-[280px] ${isExpired ? "bg-red-50 border-red-100" : "bg-white border-blue-50 shadow-xl shadow-blue-200/30"}`}
              >
                ...
                <div className="relative flex flex-col items-center space-y-5">
                  <div
                    className={`w-20 h-20 rounded-3xl flex items-center justify-center transform group-hover:rotate-12 transition-transform ${isExpired ? "bg-red-100 text-red-600" : "bg-linear-to-br from-blue-600 to-indigo-700 text-white shadow-xl shadow-blue-200"}`}
                  >
                    {subData?.expiryDate ? (
                  ...
                  <div className="text-center">
                    <p
                      className={`text-[10px] font-black uppercase tracking-widest mb-1 ${isExpired ? "text-red-400" : "text-blue-400"}`}
                    >
                      Remaining Days
                    </p>
                    <h2
                      className={`text-6xl md:text-7xl font-black tracking-tighter ${isExpired ? "text-red-600" : "text-gray-950"}`}
                    >
                      {daysLeft}
                    </h2>
                  </div>
              ...
              <div className="bg-linear-to-br from-indigo-950 via-indigo-900 to-purple-950 p-8 md:p-10 rounded-4xl text-white shadow-xl shadow-indigo-900/40 text-center space-y-6 transform hover:scale-[1.02] transition-all relative overflow-hidden group/card">
        ...
        <div className="text-center space-y-3">
          <div className="flex items-center justify-center space-x-3 mb-2">
            ...
            <p className="text-red-600 font-black text-xs uppercase tracking-widest">
              🔥 8 Restaurants are looking at this right now
            </p>
          </div>
          <h2 className="text-3xl md:text-5xl font-black text-gray-950 tracking-tighter">
            Limited Time Pricing
          </h2>
          <p className="text-gray-500 font-medium max-w-md mx-auto italic">
            Lock in these early-adopter prices before they expire next month.
          </p>
        </div>
          ...
          {plans.map((plan) => (
            ...
            >
              <div className="absolute top-8 right-8 text-right space-y-1">
                <span className="block text-[10px] text-gray-400 line-through font-bold opacity-70">
                  {plan.originalPrice}
                </span>
                <span className="block bg-emerald-100 text-emerald-700 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider">
                  {plan.saving}
                </span>
              </div>
              ...
              <div className="mb-10 text-center md:text-left">
                <div
                  className={`w-16 h-16 rounded-3xl flex items-center justify-center mb-6 bg-linear-to-br ${plan.color} text-white shadow-xl shadow-gray-200 transition-transform group-hover/plan:rotate-12 duration-500`}
                >
                  <plan.icon className="w-8 h-8" />
                </div>
                <p className="text-xs font-black tracking-[0.2em] text-blue-600 uppercase mb-2">
                  {plan.label}
                </p>
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    <span className="text-gray-400 text-xs font-bold uppercase tracking-widest opacity-70">
                      {plan.period}
                    </span>
              ...
              <div className="flex-1 space-y-4 mb-10">
                ...
                {plan.features.map((feat) => (
                  <div
                    key={feat}
                    className="flex items-start space-x-4 text-sm group/feat"
                  >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
        ...
        >
          <div className="flex items-center space-x-6">
            ...
            <div>
              <h3 className="text-lg md:text-2xl font-black text-gray-950 tracking-tight">
                VIP WhatsApp Support
              </h3>
              <p className="text-gray-400 text-sm font-medium italic">
                Instant response for our partners
              </p>
            </div>
          </div>
