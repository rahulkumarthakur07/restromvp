...
import { LayoutDashboard, Package, QrCode, LogOut, BarChart3, History, Wallet, Settings, Sun, Moon, Menu as MenuIcon, X, Users, Inbox, Hotel, CreditCard, Sparkles, ChevronRight, PanelLeftClose, PanelLeftOpen, MonitorSmartphone } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  const navItems = [
    { path: '/admin/dashboard', icon: LayoutDashboard, label: 'Live Orders', color: 'text-blue-400' },
    { path: '/admin/products', icon: Package, label: 'Manage Menu', color: 'text-emerald-400' },
    { path: '/admin/pos', icon: MonitorSmartphone, label: 'Point of Sale', color: 'text-rose-400' },
    { path: '/admin/tables', icon: QrCode, label: 'Tables & QR', color: 'text-violet-400' },
    { path: '/admin/analytics', icon: BarChart3, label: 'Analytics', color: 'text-amber-400' },
    { path: '/admin/history', icon: History, label: 'History', color: 'text-cyan-400' },
    { path: '/admin/udhar', icon: Wallet, label: 'Ledger (Udhar)', color: 'text-red-400' },
    { path: '/admin/staff', icon: Users, label: 'Staff Management', color: 'text-pink-400' },
    { path: '/admin/kitchen-inbox', icon: Inbox, label: 'Kitchen Inbox', color: 'text-orange-400' },
    { path: '/admin/cabins', icon: Hotel, label: 'Cabins & VIP', color: 'text-indigo-400' },
    { path: '/admin/settings', icon: Settings, label: 'Settings', color: 'text-gray-400' },
  ];
  ...
  return (
    <div className="h-screen bg-gray-50 flex overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-50 bg-white border-r border-gray-100 shadow-[4px_0_24px_rgba(0,0,0,0.02)] flex flex-col transform transition-all duration-300 ease-in-out md:translate-x-0 md:static ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} ${sidebarCollapsed ? 'md:w-[70px]' : 'w-64'}`}>

        {/* Logo/Brand Area */}
        <div className={`border-b border-gray-50 flex items-center ${sidebarCollapsed ? 'p-3 justify-center' : 'p-4 justify-between'}`}>
          {!sidebarCollapsed && (
            <div className="flex items-center space-x-3 min-w-0">
              {settings.logo
                ? <img src={settings.logo} className="w-9 h-9 object-contain rounded-2xl border border-white/10 shrink-0 shadow-lg" alt="Logo" />
                : <div className="w-9 h-9 rounded-2xl bg-blue-600 flex items-center justify-center shrink-0 shadow-lg"><Sparkles className="w-4 h-4 text-white" /></div>
              }
              <div className="min-w-0">
                <h2 className="text-sm font-black text-gray-950 leading-tight tracking-tight truncate">{settings.name || 'Admin Panel'}</h2>
          ...
          {/* Mobile close */}
          <button className="md:hidden p-1.5 text-gray-400 hover:text-gray-900 rounded-xl hover:bg-gray-50 transition-colors" onClick={() => setMobileMenuOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {sidebarCollapsed && (
          <div className="hidden md:flex justify-center py-3 border-b border-gray-50">
            {settings.logo
              ? <img src={settings.logo} className="w-9 h-9 object-contain rounded-2xl border border-white/10 shadow-lg" alt="Logo" />
              : <div className="w-9 h-9 rounded-2xl bg-blue-600 flex items-center justify-center shadow-lg"><Sparkles className="w-4 h-4 text-white" /></div>
            }
          </div>
        )}
              ...
              <React.Fragment key={item.path}>
                <Link
                  ...
                  className={`flex items-center px-3 py-2.5 rounded-2xl transition-all duration-300 group relative overflow-hidden ${sidebarCollapsed ? 'justify-center' : 'space-x-3'} ${
                    isActive ? 'bg-blue-50 text-blue-600 shadow-[inset_0_1px_4px_rgba(0,0,0,0.02)] border border-blue-100/50' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-950 border border-transparent'
                  }`}
                >
                  {isActive && !sidebarCollapsed && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-blue-500 rounded-r-full" />
                  )}
                  <div className={`p-1.5 rounded-xl transition-all shrink-0 ${isActive ? item.color : 'text-gray-400 group-hover:text-gray-600 group-hover:scale-110'}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  {!sidebarCollapsed && (
                    <>
                      <span className={`text-sm font-bold flex-1 truncate ${isActive ? 'text-gray-950' : ''}`}>{item.label}</span>
        ...
        <div className={`border-t border-gray-50 p-3 space-y-2`}>
          ...
          >
            ...
            <div className={`p-1.5 rounded-xl shrink-0 bg-white/20 text-white transition-transform duration-300 group-hover:scale-110 shadow-sm border border-white/10 backdrop-blur-sm`}>
              <Sparkles className="w-4 h-4" />
            </div>
            {!sidebarCollapsed && (
              <>
                <div className="flex flex-col flex-1 min-w-0">
                  <span className="text-sm font-black tracking-tight leading-tight">Upgrade Plan</span>
                  <span className="text-[10px] font-bold text-indigo-200">Unlock features</span>
                </div>
          ...
          >
            ...
            {!sidebarCollapsed && <span className="text-sm font-bold">{isDark ? 'Light Mode' : 'Dark Mode'}</span>}
          </button>
      ...
      <main className="flex-1 flex flex-col overflow-hidden min-w-0">
        ...
        <header className="md:hidden bg-white/80 backdrop-blur-xl border-b border-gray-200/50 px-4 py-3 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          ...
          <div className="flex-1 flex items-center justify-center space-x-2 overflow-hidden mx-3">
            {settings.logo && <img src={settings.logo} alt="Logo" className="w-7 h-7 rounded-lg object-contain border border-gray-100 shadow-sm shrink-0" />}
            <h2 className="font-black text-gray-950 text-sm tracking-tight truncate">{settings.name || 'Dashboard'}</h2>
          </div>
          <button
            onClick={toggleDarkMode}
